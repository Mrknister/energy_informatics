...
int read_uk_dale(UkDaleFile* file, Calibration* calib, float* voltage, float* current, int buffer_length)
{
    ...
    do {
        ...
        for (; buffer_count < read_frames; ++buffer_count) {
            voltage[total_count] = buffer[buffer_count] * voltage_multiplyer;
            ++buffer_count;
            current[total_count] = buffer[buffer_count] * amp_multiplyer;
            ++total_count;
            
            if( total_count >= buffer_length ) {
                return 1;
            }
        }
    } while (1);
...
int close_file(UkDaleFile* file)
{
    sf_close(file->snd_file);
    return 1;
}
